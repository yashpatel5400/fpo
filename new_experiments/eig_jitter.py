...
try:
    ...
    # For this script, we don't need the full dataset.py and solvers.py
    # as we are generating Gram matrix eigenvalues directly.
except ImportError:
...
def calculate_conditional_probs_p_j_given_k_torch(phi_full_torch, x_s_torch, M_val):
    ...
    # Add small epsilon to row_sums before division to prevent NaN from 0/0 if a row is all zero
...
def calculate_I_AB_components_torch(phi_params_torch, M_val, x_s_torch, q_priors_torch, eps=1e-12):
    ...
    if phi_params_torch.numel() > 0 : 
        device = phi_params_torch.device
...
def calculate_I_AB_numpy(phi_values_np, M_val, x_s_np, q_priors_np, eps=1e-12):
    """ NumPy version for evaluation. phi_values_np should be the full [phi_0, phi_1,...] """
    x_torch = torch.tensor(x_s_np, dtype=torch.float64)
    q_torch = torch.tensor(q_priors_np, dtype=torch.float64)
    if M_val == 1:
        phi_params_torch = torch.tensor([], dtype=torch.float64)
    else:
        ...
        if phi_params_np_slice.ndim == 0:
            ...
            phi_params_torch = torch.tensor(phi_params_np_slice, dtype=torch.float64)
    I_AB, _, _ = calculate_I_AB_components_torch(phi_params_torch, M_val, x_torch, q_torch, eps)
    return I_AB.item()

# --- Inner minimization for Robust Optimization ---
def inner_adversarial_objective_scipy(x_tilde_np, M_val, phi_values_rad_np, q_priors_np):
    """ Objective for SciPy: MINIMIZE I_AB(phi, x_tilde), so return I_AB. """
    I_AB_val = calculate_I_AB_numpy(phi_values_rad_np, M_val, x_tilde_np, q_priors_np)
    return I_AB_val 

def solve_inner_adversarial_problem_scipy(phi_values_rad_np, M_val, 
                                          x_center_of_uncertainty_np, # x_estimated_vals_np
                                          L2_ball_radius, 
                                          q_priors_np):
    def l2_constraint(x_tilde_np_constraint): 
        return L2_ball_radius**2 - np.sum((x_tilde_np_constraint - x_center_of_uncertainty_np)**2)
    ...
    res = minimize(inner_adversarial_objective_scipy, x0_guess, 
                   args=(M_val, phi_values_rad_np, q_priors_np),
                   method='SLSQP', bounds=bounds, constraints=constraints, 
                   options={'disp': False, 'ftol': 1e-7, 'maxiter':100})
    if res.success:
        return res.fun, res.x
    else:
        # print(f"Warning: Inner SciPy (adversarial) optimization failed. Using center value.")
        I_AB_at_center = inner_adversarial_objective_scipy(x_center_of_uncertainty_np, M_val, phi_values_rad_np, q_priors_np)
        return I_AB_at_center, x_center_of_uncertainty_np

# --- Outer Optimization (Nominal and Robust) ---
def optimize_phases_pytorch(M_val, x_for_opt_torch, q_priors_torch, 
    ...
    if M_val == 1: 
        ...
        return phi_full_np, [I_AB_val.item()] * num_epochs 
    ...
    optimizer = None
    if num_phases_to_opt > 0: optimizer = optim.Adam([phi_params], lr=lr)
    ...
    final_phi_list = [0.0] + final_phi_list_params
    return np.array(final_phi_list), history_I_AB_during_opt

def generate_random_gram_eigenvalues(M_val, ensure_sum_M=True, structured_pattern=None):
    if M_val <= 0: return np.array([])
    if M_val == 1: g_j_values = np.array([1.0])
    elif structured_pattern == 'dominant_first' and M_val >= 2:
        g_j_values = np.zeros(M_val); g_j_values[0] = 0.7 * M_val 
        remaining_sum = M_val - g_j_values[0]
        if M_val > 1 and remaining_sum > 1e-7 : 
            g_j_values[1:] = np.random.dirichlet(np.ones(M_val - 1)) * remaining_sum
        elif M_val > 1: 
            g_j_values[1:] = 1e-6 
            if np.sum(g_j_values) > 1e-9 : g_j_values = M_val * g_j_values / np.sum(g_j_values) 
            else: g_j_values = np.full(M_val, 1.0) 
    elif structured_pattern == 'uniform_like' and M_val > 0: g_j_values = np.full(M_val, 1.0) 
    else: g_j_values = np.random.dirichlet(np.ones(M_val) * 1.0) * M_val 
    g_j_values = np.maximum(1e-6, g_j_values) 
    return np.sqrt(np.sort(g_j_values)[::-1]) 
...
def run_single_experiment_config(config_tuple_with_id):
    ...
    print(f"[P{process_id}] Exp {experiment_idx+1}/{total_experiments}: "
          f"EstBias={config.get('fixed_estimation_bias_offset',0):.2f}, EstNoiseStd={config['estimation_noise_std']:.3f}, L2Rad={config['L2_uncertainty_ball_radius']:.3f}, M={config['M_states']}")
    
    num_trials_this_experiment = config['num_trials_per_experiment']
    # Changed variable name for clarity: PGM results are the 'oracle' in the sense of a fixed, known strategy
    ...
    for i_trial in range(num_trials_this_experiment):
        sqrt_g_j_true_np = generate_random_gram_eigenvalues(config['M_states'], structured_pattern=config['true_g_pattern'])
        ...
        # Create Estimated System Parameters
        ...
        elif config['estimation_error_type'] == 'invert_trend': # Kept for flexibility
        ...
        else: # Default to just random noise around true if type not specified
        ...
        else: sqrt_g_j_estimated_np = np.sqrt(generate_random_gram_eigenvalues(config['M_states'], structured_pattern='uniform_like')**2) 
        sqrt_g_j_estimated_np = np.maximum(0.01, sqrt_g_j_estimated_np)
        ...
        pgm_IAB_on_true_trials.append(I_AB_pgm_on_true) # Store PGM result
        ...
        if (i_trial + 1) % (num_trials_this_experiment // 5 or 1) == 0 :
            print(f"    [P{process_id}] Exp {experiment_idx+1}, Trial {i_trial+1}/{num_trials_this_experiment} completed. PGM IAB: {I_AB_pgm_on_true:.4f}, Nom IAB: {I_AB_phi_nom_on_true:.4f}, Rob IAB: {I_AB_phi_rob_on_true:.4f}")

    avg_pgm_IAB = np.mean(pgm_IAB_on_true_trials) # Changed from avg_oracle_err
    avg_nominal_IAB = np.mean(nominal_IAB_on_true_trials) # Changed from avg_nominal_err
    avg_robust_IAB = np.mean(robust_IAB_on_true_trials) # Changed from avg_robust_err
    ...
    if num_trials_this_experiment > 1:
        nom_valid = np.array(nominal_IAB_on_true_trials)[~np.isnan(nominal_IAB_on_true_trials)]
        rob_valid = np.array(robust_IAB_on_true_trials)[~np.isnan(robust_IAB_on_true_trials)]
        pgm_valid = np.array(pgm_IAB_on_true_trials)[~np.isnan(pgm_IAB_on_true_trials)]

        # Robust vs Nominal
        min_len_rob_nom = min(len(rob_valid), len(nom_valid))
        if min_len_rob_nom > 1:
            t_statistic_rob_gt_nom, p_value_rob_gt_nom = stats.ttest_rel(rob_valid[:min_len_rob_nom], nom_valid[:min_len_rob_nom], alternative='greater')
            if np.isnan(t_statistic_rob_gt_nom): t_statistic_rob_gt_nom = 0.0; p_value_rob_gt_nom = 1.0 if np.mean(nom_valid[:min_len_rob_nom] - rob_valid[:min_len_rob_nom]) != 0 else 0.5
        
        # Nominal vs PGM
        min_len_nom_pgm = min(len(nom_valid), len(pgm_valid))
        if min_len_nom_pgm > 1:
            t_statistic_nom_gt_pgm, p_value_nom_gt_pgm = stats.ttest_rel(nom_valid[:min_len_nom_pgm], pgm_valid[:min_len_nom_pgm], alternative='greater')
            if np.isnan(t_statistic_nom_gt_pgm): t_statistic_nom_gt_pgm = 0.0; p_value_nom_gt_pgm = 1.0 if np.mean(pgm_valid[:min_len_nom_pgm] - nom_valid[:min_len_nom_pgm]) !=0 else 0.5

        # Robust vs PGM
        min_len_rob_pgm = min(len(rob_valid), len(pgm_valid))
        if min_len_rob_pgm > 1:
            t_statistic_rob_gt_pgm, p_value_rob_gt_pgm = stats.ttest_rel(rob_valid[:min_len_rob_pgm], pgm_valid[:min_len_rob_pgm], alternative='greater')
            if np.isnan(t_statistic_rob_gt_pgm): t_statistic_rob_gt_pgm = 0.0; p_value_rob_gt_pgm = 1.0 if np.mean(pgm_valid[:min_len_rob_pgm] - rob_valid[:min_len_rob_pgm]) !=0 else 0.5
    
    result_summary = {"config": config, 
                      "avg_pgm_IAB": avg_pgm_IAB, # Changed from avg_oracle_IAB
    ...
    RESULTS_DIR_INDIVIDUAL = "results_beam_search_individual_targeted" 
    os.makedirs(RESULTS_DIR_INDIVIDUAL, exist_ok=True)
    filename = f"exp_M{config['M_states']}_bias{config.get('fixed_estimation_bias_offset',0):.2f}_noiseStd{config['estimation_noise_std']:.3f}_L2rad{config['L2_uncertainty_ball_radius']:.3f}.json"
    ...
    except Exception as e: print(f"  [P{process_id}] Exp {experiment_idx+1} FAILED to save results to {filepath}: {e}")
...
if __name__ == '__main__':
    ...
    L2_uncertainty_ball_radius_values = [0.05] 

    config_base = {
        'num_trials_per_experiment': 20, 
        'true_g_pattern': 'dominant_first', 
        'estimation_error_type': 'fixed_bias_plus_random_noise', 
        'max_pytorch_opt_epochs': 300, 
        'pytorch_lr': 0.005 
    }
    print(f"--- Starting Parameter Sweep (Parallel with L2 Ball Robustness) ---")
    print(f"True g_pattern: {config_base['true_g_pattern']}, Est. Error Type: {config_base['estimation_error_type']}")
    print(f"Sweeping over: M_states={NUM_DISTINCT_STATES_M_SWEEP}, fixed_bias={fixed_bias_offset_values}, est_noise_std={estimation_noise_std_values}, L2_radius={L2_uncertainty_ball_radius_values}")
    ...
    for i_exp, (n_states, bias_offset, est_noise_std, l2_rad) in enumerate(param_combinations):
        ...
        current_config['k_psi0_modes'] = min_k_for_n_states # This is for generate_distinct_single_fourier_modes if used by setup_multistate_evolution
    ...
    num_processes = min(multiprocessing.cpu_count(), len(param_configurations_for_pool), 32) 
    ...
    if num_processes > 0 and len(param_configurations_for_pool) > 0:
        ...
        for config_tuple_idx, config_tuple_val in enumerate(param_configurations_for_pool):
            config_tuple_with_pid = (config_tuple_val[0], config_tuple_val[1], config_tuple_val[2], os.getpid()) 
            all_results_summary_list.append(run_single_experiment_config(config_tuple_with_pid))
    ...
    print("\n\n--- Parameter Sweep Results Summary (Targeted Misleading Estimate) ---")
    ...
    RESULTS_DIR_SWEEP = "results_beam_search_summary_targeted" 
    os.makedirs(RESULTS_DIR_SWEEP, exist_ok=True)
    csv_path = os.path.join(RESULTS_DIR_SWEEP, f"param_sweep_summary_L2_Robust_Targeted_with_PGM.csv")
